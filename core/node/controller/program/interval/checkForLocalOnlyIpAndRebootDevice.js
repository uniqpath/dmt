...
import { log, colors, isRPi, disconnectedIPAddress, isDevUser, stripAnsi, globals } from 'dmt/common';
  ...
  if (ip && isRPi()) {
    if (disconnectedIPAddress(ip)) {
      ...
      if (prevIp != ip && disconnectedIPAddress(prevIp)) {
        const msg = `✅ Device received a valid IP address [ ${colors.gray(prevIp)} → ${ip} ].`;
        log.green(msg);
