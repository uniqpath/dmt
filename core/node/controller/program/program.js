...
class Program extends EventEmitter {
  constructor({ mids, fromABC }) {
    ...
    this.on('ready', () => {
      if (dmt.isMainDevice()) {
        for (const device of dmt.devices()) {
          if (device._id && device.deviceName != device._id) {
            log.yellow(
              `⚠️ Ignoring device name from ${colors.cyan(`${device.id}/device.def (${colors.red(device._id)})`)}, deviceName is ${colors.cyan(
                device.deviceName
              )} ${colors.gray(`(determined by directory)`)}`
            );
          }
        }
      }
  ...
  loadDirectory(dir, filter, { debug = false } = {}) {
    load(this, dir, filter, false, debug);
  }

  loadDirectoryRecursive(dir, filter, { debug = false } = {}) {
    load(this, dir, filter, true, debug);
  }
  ...
  isHub() {
    if (!dmt.apMode()) {
      ...
      return definedIp == accessPointIP || assignedIp == accessPointIP;
    }
  }
  ...
  downstream(pubKeyHex) {
    return this.acceptor.downstream(pubKeyHex)?.remoteObject('downstream');
  }
  ...
  showNotification(
    ...
    if (title) {
      ...
      recommendedTitle = this.device.deviceName;
    }
    ...
    if (!clearNotification) {
      ...
      if (dmt.isPersonalComputer() && !omitDesktopNotification) {
        ...
        desktop.notify(_msg || '[ missing message ]', `${recommendedTitle || this.network.name() || this.device.deviceName}`.trim());
      }
    }
  }
