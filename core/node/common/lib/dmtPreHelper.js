...
function device({ deviceName = 'this', onlyBasicParsing = false, caching = true } = {}) {
  ...
  if (deviceName == 'this') {
    ...
    const deviceDir = path.join(dmtUserDir, 'devices/this');
    const realDeviceDir = fs.realpathSync(deviceDir);
    const deviceId = realDeviceDir.replace(`${path.join(dmtUserDir, 'devices')}/`, '');
    const _deviceName = deviceId.split('/')[deviceId.split('/').length - 1];

    return { ...readDeviceDef({ filePath, onlyBasicParsing, caching }), deviceName: _deviceName, id: deviceId };
  }
...
function devices({ onlyBasicParsing = false, caching = true, devicesDir = path.join(dmtUserDir, 'devices'), recursive = false } = {}) {
  if (!recursive) {
  ...
  for (const deviceDir of list) {
    const deviceId = deviceDir.replace(`${path.join(dmtUserDir, 'devices')}/`, '');

    let _coredata;

    if (!onlyBasicParsing) {
      _coredata = { deviceDir: deviceId };
    ...
    if (fs.existsSync(deviceDefFile)) {
      ...
      def.deviceName = deviceId.split('/')[deviceId.split('/').length - 1];
      ...
      const hasSubdirectory = dirPath => {
        const entries = fs.readdirSync(dirPath, { withFileTypes: true });
        return entries.some(entry => entry.isDirectory());
      };

      if (hasSubdirectory(deviceDir)) {
        devices({ onlyBasicParsing, caching, devicesDir: deviceDir, recursive: true });
      }
    }
  }
