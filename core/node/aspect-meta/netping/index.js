...
function reportConnectivityOnLan() {
  return isRPi() || device().id == 'eclipse';
}
...
function init(program) {
  ...
  wanConnectivity.on('connection_lost', ({ code }) => {
    ...
    program.emit('internet_connection_lost');

    if (device().id == 'eclipse' || isDevMachine() || !isPersonalComputer()) {
        ...
        setTimeout(() => {
          ...
          if (!wokeAt || Date.now() - wokeAt > 5000) {
            if (Date.now() - lastIntervalAt <= 2.1 * globals.tickerPeriod) {
              ...
              if (device().id == 'eclipse' || isDevMachine()) {
                log.green(`PC Wake up after ${Math.round((Date.now() - lastIntervalAt) / 1000)}s`);
