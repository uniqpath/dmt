...
import { colors, dmtUserDir, log, device, def } from 'dmt/common';
...
async function init(program) {
  function userEngineReady(results) {
    deviceLoader(program);

    const notificationsDisabled = device().notifications == 'false';

    const notificationsDir = path.join(userEnginePath, '_notifications');

    if (fs.existsSync(notificationsDir)) {
      if (notificationsDisabled) {
        log.magenta('Not loading dmt notifications because they are disabled in device.def');
      } else {
