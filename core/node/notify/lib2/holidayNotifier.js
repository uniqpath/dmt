...
class HolidayNotifier extends ScopedNotifier {
  constructor(
    { country, symbol, title, color, ttl, highPriority, notifyAt = NOTIFIER_DEFAULT_TIME, app = DEFAULT_APP, notifyDaysBefore = 1, user, users } = {},
...
export default function holidayNotifier(options = {}) {
  const decommissionable = isReloadableNotifications(new Error(), import.meta.url);

  return new HolidayNotifier(options, decommissionable);
}
