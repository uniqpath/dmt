...
import getTimepoint from './lib/getTimepoint.js';
...
const { monthsAgo, ONE_HOUR } = timeutils;
...
class DateNotifier extends ScopedNotifier {
  constructor(
    ...
    this.disableCountdownWarning = disableCountdownWarning;
  ...
  checkNotificationTimes(entry, fakeNow) {
    ...
    const {
      ...
      disableCountdownWarning: _disableCountdownWarning,
    ...
    const firstYear = _firstYear || this.firstYear;
    ...
    let notifyDays = _notifyDays || this.notifyDays;

    if (notifyDays) {
      notifyDays = Array.isArray(notifyDays) ? notifyDays : [notifyDays];
    }

    const repeatDays = _repeatDays || this.repeatDays;

    let repeatFinalDaysAfter;

    if (repeatDays) {
      repeatFinalDaysAfter = true;
      notifyDays = Array.isArray(repeatDays) ? repeatDays : [repeatDays];
    }

    if (notifyDays) {
      for (let i = 1; i < notifyDays.length; i++) {
        if (notifyDays[i] <= notifyDays[i - 1]) {
          throw new Error(`${title} - ${msg} - notifyDays / repeatDays numbers should increase monotonically and here is not: ${JSON.stringify(notifyDays)}`);
        }
      }
    }
    ...
    if (!_symbol) {
      _symbol = notifyDays ? SCROLL_SYMBOL : CALENDAR_SYMBOL;
    }

    const uniqueArray = arr => [...new Set(arr)];

    const omitAtEventTime = _omitAtEventTime == undefined ? this.omitAtEventTime : _omitAtEventTime;
    const disableCountdownWarning = _disableCountdownWarning == undefined ? this.disableCountdownWarning : _disableCountdownWarning;
    ...
    const omitOnDayOfEvent = !!__notifyDaysBefore.find(x => x == -1 || x == null) || (omitAtEventTime && notifyMinutesBefore.length == 0);
    ...
    let list = when.map(t => getTimepoint({ t, defaultTime: defaultTime || NOTIFIER_DEFAULT_TIME, defaultYear }));
    ...
    if (isPastEvent && notifyDays) {
      const { timepoint, time, isUnspecifiedTime } = mostRecentEvent;

      const HOURS = 10;
      const SECOND_MSG_MIN = HOURS * 60;

      list = [];

      for (const days of notifyDays) {
        list.push({ timepoint: addDays2(timepoint, days), time, isUnspecifiedTime, __ttl: 10 * ONE_HOUR });
        list.push({ timepoint: addMinutes(addDays2(timepoint, days), SECOND_MSG_MIN), time, isUnspecifiedTime });
      }

      if (repeatFinalDaysAfter) {
        const lastDayAfter = notifyDays[notifyDays.length - 1];

        for (let i = 2; i < 100; i++) {
          list.push({ timepoint: addDays2(timepoint, i * lastDayAfter), time, isUnspecifiedTime, __ttl: 10 * ONE_HOUR });
    ...
    for (const { timepoint, time, isUnspecifiedTime, __ttl } of list) {
      if (everyNthYear && (timepoint.getFullYear() - firstYear || 0) % everyNthYear != 0) {
        continue;
      }
        ...
        if (!omitOnDayOfEvent) {
          ...
          minutesBefore.forEach((min, index) => {
            ...
            if (isSameMinute(now, notificationTime)) {
              ...
              const symbol =
                !notifyDays && isLastNotification && !entry.hasRandomPeriodicEvents && (importantEvent || this.isLastEvent({ list, timepoint }))
              ...
              this.handleMessage(prepareAndPrehashObj({ obj, msg, now, delayWarning: !(isPastEvent && notifyDays) }));
            }
          });
        }
          ...
          notifyDaysBefore
            ...
            .forEach((daysBefore, index) => {
              if (isSameDay(timepoint, addDays(now, daysBefore))) {
                ...
                } else if (notifyDayBeforeAt.length == 0) {
                  times.push(time);
                } else {
                  times.push(
                    notifyDayBeforeAt.sort((a, b) => {
                ...
                for (const t of times) {
                  ...
                  if (isSameMinute(now, notificationTime)) {
                    ...
                    if (!disableCountdownWarning && notifyDaysBefore.length >= 2) {
