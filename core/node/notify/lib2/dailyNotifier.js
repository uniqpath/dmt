...
class DailyNotifier extends ScopedNotifier {
  constructor(notifications, { symbol = '🔔', color, ttl, highPriority, title, warnAfter, user } = {}) {
    super(`${symbol} ${title || ''}`);
  ...
  checkNotificationTimes(entry) {
    ...
    const { msg: msgStrOrArray, warnAfter: _warnAfter, symbol: _symbol, when, id, color, ttl, from, until, highPriority: _highPriority, user: _user } = entry;
