...
export default function notify(obj) {
  return new Promise((success, reject) => {
    ...
    if (obj.notifyAll && (obj.group?.length > 1 || userKey?.length > 0 || user?.length > 0)) {
      const promises = [];

      promises.push(notify({ ...obj, notifyAll: false }));
      promises.push(notify({ ...obj, userKey: [], user: [], group: null }));
