...
module.exports.fileCommandJson = function(notifier, options, cb) {
  ...
  const n = cp.spawn(notifier, options);

  n.on('close', code => {
    if (code != 0) {
      console.log(`terminal-notifier process exited with code ${code}`);
    }
  });
};
...
const allowedToasterFlags = ['t', 'm', 'b', 'tb', 'p', 'id', 's', 'silent', 'appID', 'pid', 'pipeName', 'close', 'install'];
