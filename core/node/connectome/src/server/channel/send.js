...
function send({ message, channel }) {
  ...
  if (channel.verbose) {
    if (channel.sharedSecret) {
      logger.write(log, `Channel ${channel.remoteAddress()} → Sending encrypted message #${channel.sentCount} @ ${channel.remoteAddress()}:`);
    } else {
      logger.write(log, `Channel ${channel.remoteAddress()} → Sending message #${channel.sentCount} @ ${channel.remoteAddress()}:`);
    }
