...
// (not ulitized) https://crates.io/crates/unidecode
// we need to do this with regex to be able to indicate where the match was
...
fn escape(x: &str) -> String {
  ...
  // might not be completely in sync with the amazing dmt
  //let result =
  //return unidecode(&escaped);

  let letters = "[cçćč]";
  let re = Regex::new(&format!("(?i){}", letters)).unwrap();
  let result = re.replace_all(&escaped, letters);

  let letters = "[sšßś]";
  let re = Regex::new(&format!("(?i){}", letters)).unwrap();
  let result = re.replace_all(&result, letters);
  ...
  let letters = "[aàáâäæãåāa]";
  let re = Regex::new(&format!("(?i){}", letters)).unwrap();
  let result = re.replace_all(&result, letters);

  let letters = "[eèéêëēėęę]";
  let re = Regex::new(&format!("(?i){}", letters)).unwrap();
  let result = re.replace_all(&result, letters);
      ...
      None => {
        ...
        for entry in WalkDir::new(".") {
            ...
            if !onlyFiles || !md.is_dir() {
                ...
                // we cannot do it like this if we want to keep highlighting information.. to color where text matched
                // for example search 'z' and ŽivaŽanŽan -> Ž's are blue .. we achieve this with let c = "[cćč]"; ... code
                // with just replacing lines and search terms with unidecode we loose that capability :/
                // searching for 'cachu' in Cachúmbala won't be able to color "Cachú" in blue !
                //if let Some(result) = line_matches(unidecode(&line), color, &regs, &neg_regs, &temp_re) {
                if let Some(result) = line_matches(line, color, &regs, &neg_regs, &temp_re) {
                    if searchAbsolutePath {
                        println!("{}", &result);
                    } else {
